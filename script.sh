...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='LebJe_LFSPointers'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/LebJe_LFSPointers.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='LebJe_LFSPointers'
  ...
  echo "Generating yum cache for LebJe_LFSPointers..."
  yum -q makecache -y --disablerepo='*' --enablerepo='LebJe_LFSPointers'

  echo "Generating yum cache for LebJe_LFSPointers-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='LebJe_LFSPointers-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh LebJe_LFSPointers
  zypper --gpg-auto-import-keys refresh LebJe_LFSPointers-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/LebJe/LFSPointers/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/LebJe_LFSPointers.repo
  else
    yum_repo_path=/etc/yum.repos.d/LebJe_LFSPointers.repo
  fi
